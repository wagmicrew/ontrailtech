...
export default function AppWindow({ win }: { win: WindowState }) {
  ...
  const glassStyle: React.CSSProperties = prefs.glassmorphism
    ? {
        ...
        backgroundColor: `rgba(18, 20, 28, ${glassAlpha})`,
        border: prefs.showBorder ? '1px solid rgba(255,255,255,0.10)' : 'none',
      }
    : {
        backgroundColor: 'rgba(24, 26, 36, 0.97)',
        border: prefs.showBorder ? '1px solid rgba(255,255,255,0.08)' : 'none',
      };

  const titlebarStyle: React.CSSProperties =
    prefs.titlebarcStyle === 'glass'
      ? { backdropFilter: `blur(${prefs.blurAmount + 4}px)`, WebkitBackdropFilter: `blur(${prefs.blurAmount + 4}px)`, backgroundColor: 'rgba(255,255,255,0.04)', borderBottom: '1px solid rgba(255,255,255,0.08)' }
      : prefs.titlebarcStyle === 'minimal'
      ? { backgroundColor: 'transparent', borderBottom: '1px solid rgba(255,255,255,0.06)' }
      : { backgroundColor: 'rgba(12,14,20,0.6)', borderBottom: '1px solid rgba(255,255,255,0.08)' };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-xs font-semibold text-white/60 flex-1 truncate tracking-wide">{win.title}</span>
        ...
        <div className="flex items-stretch h-full flex-shrink-0" onMouseDown={e => e.stopPropagation()}>
          <button
            ...
            className="flex items-center justify-center w-9 h-full text-white/35 hover:text-white/80 hover:bg-white/10 transition-colors duration-100 border-l border-white/[0.06]"
          >
            <MinimizeIcon />
          </button>
          <button
          ...
          <button
            ...
            className="flex items-center justify-center w-9 h-full text-white/35 hover:text-red-400 hover:bg-red-500/20 transition-colors duration-100 border-l border-white/[0.06]"
          >
            <CloseIcon />
          </button>
        </div>
      ...
      <div
        ...
        style={{ background: prefs.glassmorphism ? 'rgba(255,255,255,0.02)' : 'rgba(255,255,255,0.01)' }}
      >

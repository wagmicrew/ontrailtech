...
// â”€â”€ Smooth Vertical Sidebar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Smooth Drawer â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    >
      <motion.div variants={itemV} className={`flex items-center gap-3 px-4 py-4 border-b ${t.border} flex-shrink-0`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className={`text-xs ${statusColor[app.status] ?? t.textMuted}`}>v{app.version} Â· {app.status}</p>
        </div>
...
function UploadZone({ onUpload, uploading, t }: {
  ...
  return (
    ...
    >
      ...
      {uploading
        ? <p className="text-xs text-indigo-400 font-medium">Uploadingâ€¦</p>
        : <><p className={`text-xs font-medium ${t.text}`}>Drop a <code className="font-mono">.app</code> file</p><p className={`text-[10px] ${t.textMuted} mt-0.5`}>or click to browse Â· max 10 MB</p></>
      }
    </div>
  );
}
          ...
          <div className="grid gap-1.5" style={{ gridTemplateColumns: `repeat(${row.opts.length}, 1fr)` }}>
            {row.opts.map(o => (
              <button key={o} onClick={() => (setWin as (k: string, v: string) => void)(row.key, o)} className={chip(winPrefs[row.key] === o)}>
                {(row.labels as unknown as Record<string, string>)[o]}
              </button>
            ))}
          </div>
        </div>
    ...
    <div className="space-y-5">
      ...
      <div className={`rounded-xl border ${t.border} ${t.bgCard} divide-y ${t.divider} px-4`}>
        {[
          ...
          { label: 'Browser',    value: navigator.userAgent.slice(0, 36) + 'â€¦', cls: t.text },
        ].map(row => (
...
function AppsPanel({ t }: { t: ReturnType<typeof useTheme> }) {
  ...
  async function handleUpload(file: File) {
    ...
    try {
      ...
      setApps(p => [app, ...p]);
      setSelected(app);
      showToast(`"${app.name}" uploaded â€” click Install.`);
        ...
        {apps.map(app => (
          <button key={app.app_id} onClick={() => setSelected(selected?.app_id === app.app_id ? null : app)}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className={`text-[10px] ${t.textMuted} truncate`}>v{app.version}{app.author ? ` Â· ${app.author}` : ''}</p>
            </div>
...
// â”€â”€ Content slide transition â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

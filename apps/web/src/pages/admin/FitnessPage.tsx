...
const PROVIDERS: FitnessProvider[] = [
  {
    ...
    icon: '🚴',
    color: 'orange',
    docUrl: 'https://developers.strava.com/',
    fields: [
      { key: 'client_id', label: 'Client ID', hint: 'From Strava API settings' },
      { key: 'client_secret', label: 'Client Secret', sensitive: true, hint: 'From Strava API settings' },
      { key: 'webhook_verify_token', label: 'Webhook Verify Token', sensitive: true, hint: 'Your custom secret for webhook validation' },
    ],
  },
  {
    id: 'samsung_health',
    ...
    icon: '💙',
    color: 'blue',
    docUrl: 'https://developer.samsung.com/health',
    fields: [
      { key: 'app_id', label: 'App ID', hint: 'Samsung Health Partner App ID' },
      { key: 'app_secret', label: 'App Secret', sensitive: true, hint: 'Samsung Health Partner App Secret' },
    ],
  },
  {
    id: 'apple_health',
    ...
    icon: '🍎',
    color: 'red',
    docUrl: 'https://developer.apple.com/health-fitness/',
    fields: [
      { key: 'team_id', label: 'Apple Team ID', hint: 'From Apple Developer account' },
      { key: 'bundle_id', label: 'Bundle ID', hint: 'e.g. tech.ontrail.app' },
      { key: 'key_id', label: 'Key ID (AuthKey)', sensitive: true },
      { key: 'private_key', label: 'Private Key (.p8 content)', sensitive: true, hint: 'Paste full key including headers' },
    ],
  },
  {
    id: 'ontrail',
    name: 'OnTrail App (Own)',
    icon: '🏃',
    color: 'green',
    docUrl: '',
    fields: [
      { key: 'webhook_secret', label: 'Webhook Secret', sensitive: true, hint: 'Shared secret for internal app sync' },
      { key: 'api_key', label: 'Internal API Key', sensitive: true, hint: 'Key used by the mobile app to push fitness data' },
...
export default function FitnessPage() {
  ...
  useEffect(() => {
    adminFetch('/admin/fitness/config')
      .then(data => {
        setConfigs(data.configs || {});
        setEnabled(data.enabled || {});
      })
      .catch(() => {});
    adminFetch('/admin/fitness/stats')
      .then(setStepsStats)
      .catch(() => {});
  }, []);

  const setField = (providerId: string, key: string, value: string) => {
    setConfigs(c => ({ ...c, [providerId]: { ...(c[providerId] || {}), [key]: value } }));
  };

  const saveProvider = async (providerId: string) => {
    setErrors(e => ({ ...e, [providerId]: '' }));
    try {
      await adminFetch(`/admin/fitness/config/${providerId}`, {
        method: 'PUT',
        body: JSON.stringify({ config: configs[providerId] || {}, enabled: enabled[providerId] ?? false }),
      });
      setSaved(s => ({ ...s, [providerId]: true }));
      setTimeout(() => setSaved(s => ({ ...s, [providerId]: false })), 3000);
    } catch (e: any) {
      setErrors(er => ({ ...er, [providerId]: e.message }));
    }
  };

  const webhookBase = `${API}/fitness/webhook`;
    ...
    <div className="space-y-6">
      ...
      {stepsStats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { label: 'Total synced users', value: stepsStats.synced_users ?? '—' },
            { label: 'Steps today (all)', value: stepsStats.steps_today?.toLocaleString() ?? '—' },
            { label: 'Rep points earned today', value: stepsStats.rep_today?.toFixed(0) ?? '—' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {PROVIDERS.map(provider => {
          ...
          return (
            <div key={provider.id} className={`bg-white border rounded-2xl shadow-sm overflow-hidden ${isEnabled ? `border-${provider.color}-200` : 'border-gray-100'}`}>
              {/* Header */}
              <div className={`px-5 py-4 flex items-center justify-between ${isEnabled ? c.bg : 'bg-gray-50'} border-b ${isEnabled ? c.border : 'border-gray-100'}`}>
                ...
                <div className="flex items-center gap-2">
                  <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${isEnabled ? c.badge : 'bg-gray-100 text-gray-500'}`}>
                    {isEnabled ? 'Enabled' : 'Disabled'}
                  </span>
                  <button
                    onClick={() => setEnabled(e => ({ ...e, [provider.id]: !isEnabled }))}
              ...
              <div className="px-5 py-4 space-y-3">
                {provider.fields.map(field => (
                  <div key={field.key}>
                    ...
                    <div className="relative">
                      <input
                        type={field.sensitive && !showSecret[`${provider.id}.${field.key}`] ? 'password' : 'text'}
                        value={cfg[field.key] || ''}
                      ...
                      {field.sensitive && (
                        <button type="button"
                          onClick={() => setShowSecret(s => ({ ...s, [`${provider.id}.${field.key}`]: !s[`${provider.id}.${field.key}`] }))}
                          className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                          {showSecret[`${provider.id}.${field.key}`] ? (
                ...
                <div className="mt-2 pt-3 border-t border-gray-100">
                  ...
                  <div className="flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-2">
                    <code className="text-xs text-gray-600 font-mono flex-1 truncate">{webhookBase}/{provider.id}</code>
                    <button onClick={() => navigator.clipboard.writeText(`${webhookBase}/${provider.id}`)}
                      className="text-gray-400 hover:text-gray-600 flex-shrink-0" title="Copy">
      ...
      {/* Steps → Reputation info box */}
      <div className="bg-green-50 border border-green-200 rounded-2xl p-5">
        <h3 className="font-semibold text-green-800 text-sm mb-2">📊 Steps → Reputation Pipeline</h3>
        <p className="text-sm text-green-700 leading-relaxed">
          When a connected fitness provider syncs step data, the engine converts steps to reputation points using the configured
          <strong> steps_per_reputation</strong> ratio (OnTrail App settings). Each sync event is recorded as a
          <code className="mx-1 text-xs bg-green-100 px-1 rounded">reputation_event</code>
          and the user's score updates in real-time. Samsung Health and Strava also emit webhook payloads for activity updates
          which trigger the same pipeline.
        </p>
      </div>
    </div>
  );

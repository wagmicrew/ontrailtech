...
export default function UsersPage() {
  ...
  const saveUser = async () => {
    if (!editing) return;
    try {
      await adminFetch(`/admin/users/${editing.id}`, {
        method: 'PATCH',
  ...
  const revokeSession = async (sessionId: string) => {
    try {
      await adminFetch(`/admin/sessions/${sessionId}`, { method: 'DELETE' });
      if (sessionUser) loadSessions(sessionUser);
    } catch (e: any) {
      setError(e.message);
    }
  };
  ...
  const startEdit = (u: UserRow) => {
    setEditing(u);
    setEditForm({ username: u.username || '', email: u.email || '', roles: [...u.roles] });
  };
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    {['User', 'Email', 'Wallet', 'Roles', 'Rep. Score', 'Joined', 'Actions'].map(h => (
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <button onClick={() => startEdit(u)}
                            className="p-1.5 rounded-md text-gray-400 hover:text-green-600 hover:bg-green-50 transition-colors" title="Edit">
                            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931z" />
                            </svg>
                          </button>
                          <button onClick={() => { setTab('sessions'); loadSessions(u.id); }}
                            className="p-1.5 rounded-md text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition-colors" title="Sessions">
                            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
            ...
            <div className="px-4 py-3 border-t border-gray-100 flex items-center justify-between bg-gray-50/50">
              ...
              <div className="flex gap-2">
                <button disabled={page === 0} onClick={() => setPage(p => p - 1)}
                  className="px-3 py-1.5 rounded-lg text-xs bg-white border border-gray-200 disabled:opacity-40 hover:bg-gray-50 transition-colors">← Prev</button>
                <button disabled={users.length < PAGE_SIZE} onClick={() => setPage(p => p + 1)}
                  className="px-3 py-1.5 rounded-lg text-xs bg-white border border-gray-200 disabled:opacity-40 hover:bg-gray-50 transition-colors">Next →</button>
              </div>
            </div>
          </div>
        </>
      ...
      {tab === 'sessions' && (
        <div className="bg-white border border-gray-100 rounded-xl shadow-sm">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <button onClick={() => setTab('users')} className="text-sm text-gray-400 hover:text-gray-600 transition-colors">← Back to users</button>
          </div>
          {sessions.length === 0 ? (
            <div className="px-6 py-10 text-center text-gray-400 text-sm">
              {sessionUser ? 'No active sessions for this user' : 'Select a user to view their sessions'}
            </div>
          ) : (
            <table className="w-full text-sm">
              <thead><tr className="bg-gray-50 border-b border-gray-100">
                {['Token (partial)', 'IP Address', 'Created', 'Expires', ''].map(h => (
                  <th key={h} className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">{h}</th>
                ))}
              </tr></thead>
              <tbody className="divide-y divide-gray-50">
                  ...
                  <tr key={s.id} className="hover:bg-gray-50/60">
                    ...
                    <td className="px-4 py-3">
                      <button onClick={() => revokeSession(s.id)}
                        className="px-3 py-1 text-xs bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors font-medium">Revoke</button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex gap-3 justify-end">
              ...
              <button onClick={saveUser}
                className="px-5 py-2 bg-green-500 hover:bg-green-600 text-white text-sm font-medium rounded-lg transition-colors">Save changes</button>
            </div>
          </div>
        </div>
      )}

...
- [x] 1. Data models and database migration
  ...
  - [x] 1.2 Create Alembic migration for aura tables
  ...
  - [x] 1.3 Seed default aura config keys in `admin_config`
...
- [x] 2. Add Redis cache constants and aura config helpers
...
- [x] 3. Checkpoint — Ensure models and migration are correct
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. Implement Aura Engine core (`services/api/engines/aura_engine.py`)
  - [x] 4.1 Create `aura_engine.py` with config loader and core calculation
  ...
  - [x] 4.4 Implement aura level classification
  ...
  - [x] 4.5 Implement recalculation queue and batching
  ...
  - [x] 4.6 Implement multiplier functions for other engines
...
- [ ] 6. Implement Ancient NFT Indexer (`services/api/engines/ancient_indexer.py`)
  ...
  - [x] 6.2 Register indexer as FastAPI startup background task in `main.py`
    - _Requirements: 1.1_

  - [x] 6.3 Add Ancient NFT contract ABI to `web3_client.py`
...
- [x] 7. Implement recalculation triggers in existing engines
  - [x] 7.1 Add aura recalculation hooks to `event_indexer.py`
  ...
  - [x] 7.2 Add aura recalculation hook to token buy/sell in `routers/tokens.py`
    - After share buy/sell, check if trader is Ancient holder → `enqueue_recalculation(runner_id)`
    - _Requirements: 3.3_

  - [x] 7.3 Add aura recalculation hook to `reputation_engine.py`
    - After reputation score change, if user is Ancient holder → enqueue recalc for all runners they support
    - _Requirements: 3.4_

  - [x] 7.4 Add aura recalculation from Ancient NFT Transfer events in `ancient_indexer.py`
    - On holder status change → recalculate aura for all runners that wallet supported
    - _Requirements: 3.5_

- [x] 8. Integrate aura multipliers into existing engines
  - [x] 8.1 Modify `bonding_curve_price()` in `routers/tokens.py` to accept effective supply
  ...
  - [x] 8.2 Modify `buy_shares()` in `routers/tokens.py` to apply aura boost to token allocation
    - Compute `allocated = amount × (1 + auraBoost)` for aura-backed runners
    - Log aura boost in `token_transaction.event_metadata`
    - _Requirements: 5.1, 5.2, 5.3, 5.4_

  - [x] 8.3 Modify TGE threshold check to use effective tips
    - Compare `effectiveTips` (from `get_effective_tips()`) against threshold
    - _Requirements: 6.1, 6.2, 6.3, 6.4_

  - [x] 8.4 Modify `record_reputation_event()` in `reputation_engine.py` to apply aura factor
...
- [x] 10. Implement Influence Graph Engine (`services/api/engines/influence_engine.py`)
  - [x] 10.1 Create `influence_engine.py` with core functions
  ...
  - [x] 10.2 Add influence edge creation hooks to existing event flows
...
- [x] 11. Implement Aura API Router (`services/api/routers/aura.py`)
  - [x] 11.1 Create `aura.py` router with runner aura endpoint
  ...
  - [x] 11.2 Add leaderboard endpoints
  ...
  - [x] 11.3 Register aura router in `main.py`
...
- [x] 12. Implement Graph API Router (`services/api/routers/graph.py`)
  - [x] 12.1 Create `graph.py` router with graph endpoints
  ...
  - [x] 12.2 Register graph router in `main.py`
...
- [x] 13. Implement EIP-712 signing for aura-adjusted parameters
...
- [x] 14. Implement admin config management for aura
  - [x] 14.1 Add aura config update endpoint to `routers/admin.py`
...
- [x] 15. Implement aura serialization with round-trip consistency
...
- [x] 17. Implement frontend AuraIndicator component
  - [x] 17.1 Create `apps/web/src/components/AuraIndicator.tsx`
  ...
  - [x] 17.2 Integrate AuraIndicator into runner profile pages
    - Add to `RunnerLanding.tsx` and `Profile.tsx`
    - _Requirements: 10.6_

- [x] 18. Implement AuraRings overlay component
...
- [x] 19. Implement Aura Leaderboard page
  - [x] 19.1 Create `apps/web/src/pages/AuraLeaderboard.tsx`
  ...
  - [x] 19.2 Add leaderboard route to `apps/web/src/pages/Routes.tsx` (app router)
    - _Requirements: 11.1_

- [x] 20. Implement Influence Graph visualization
  - [x] 20.1 Update `apps/web/src/components/InfluenceGraph.tsx`
  ...
  - [x] 20.2 Add "Follow the Alpha" discovery action
    - Navigate to high-aura clusters from trending endpoint
    - _Requirements: 26.2_

- [x] 21. Implement Aura Feedback system in frontend
